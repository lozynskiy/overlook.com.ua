...
class ModelCheckoutOrder extends Model {
	...
	public function getOrderProduct($order_id) {
		$query = $this->db->query("SELECT op.*, p.sku, (SELECT GROUP_CONCAT(CONCAT(oo.name, ': ', oo.value) SEPARATOR ', ') FROM " . DB_PREFIX . "order_option oo WHERE oo.order_id = op.order_id and oo.order_product_id = op.order_product_id GROUP BY oo.order_product_id) AS options_data, (SELECT md.name FROM " . DB_PREFIX . "manufacturer_description md WHERE md.manufacturer_id = p.manufacturer_id AND md.language_id = '" . (int)$this->config->get('config_language_id') . "' ) AS manufacturer, (SELECT cd.name FROM " . DB_PREFIX . "product_to_category pc INNER JOIN " . DB_PREFIX . "category_description cd on cd.category_id = pc.category_id WHERE cd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND pc.product_id = op.product_id LIMIT 1) AS category FROM " . DB_PREFIX . "order_product op INNER JOIN " . DB_PREFIX . "product p on p.product_id = op.product_id WHERE op.order_id = '" . (int)$order_id . "'");
		return $query->rows;
	}
