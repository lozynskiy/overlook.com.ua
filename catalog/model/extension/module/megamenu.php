...
class ModelExtensionModuleMegamenu extends Model {
	...
	public function parseHtml($item)
	{
		...
		if ($result['thumb']) {
			$result['thumb'] = $this->model_tool_image->resize($result['thumb'], 206, 122);
		}
			...
			foreach($item['options']['manufacturers_list'] as $manufacturer_id) {
				...
				if ($manufacturer) {
					...
					if (is_file(DIR_IMAGE . $manufacturer['image'])) {
						$thumb = $this->model_tool_image->resize($manufacturer['image'], 206, 122);
					}
					else {
						$thumb = $this->model_tool_image->resize('no_image.png', 206, 122);
					}
				...
				if ($category) {
					...
					if ($result['subtype'] == "full_image") {
						if (is_file(DIR_IMAGE . $category['image'])) {
							$thumb = $this->model_tool_image->resize($category['image'], 206, 122);
						}
						else {
							$thumb = $this->model_tool_image->resize('no_image.png', 206, 122);
						}
