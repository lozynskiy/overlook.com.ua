	...
	<?php foreach ($products as $product) { $row_style_background = ($i++ % 2) ? "#f6f6f6" : "#fafafa"; ?>
	    <tr>
			<td bgcolor="<?php echo $row_style_background; ?>">
				...
				<?php if ($product['stock_quantity'] !== false && $product['stock_subtract']) { ?><br /><b><?php echo $text_stock_quantity; ?>:</b> <span style="color: <?php if ($product['stock_quantity'] <= 0) { echo '#FF0000'; } elseif ($product['stock_quantity'] <= 5) { echo '#FFA500'; } else { echo '#008000'; }?>"><?php echo $product['stock_quantity']; ?></span><?php } ?>
			</td>
			<?php if ($config['table_quantity']) { ?>
...
<?php if (!empty($order_link)) { ?>
	<div class="link"><b><?php echo $text_order_link; ?></b><br /><span class="icon">&raquo;</span><a href="<?php echo $order_link; ?>" target="_blank"><b><?php echo $order_link; ?></b></a></div>
<?php } ?>
