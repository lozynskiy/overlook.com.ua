...
class Stat{
	...
	*/
	function createCampaign($sender, $text, $list_id, $datetime, $batch, $batchinterval, $sms_lifetime, $controlnumber=NULL){
	...
	*/
	function getCampaignList($from, $offset, $direction) {
		return $this->gateway->execCommad('getCampaignList',array('from'=>$from, 'offset'=>$offset, 'orderby'=>$direction));
	}
    
	/*
	**	gets list of campaigns
	*/
	function getCampaignListExt($from, $limit, $orderfield, $direction, $search) {
		return $this->gateway->execCommad('getCampaignListExt',array('from'=>$from, 'limit'=>$limit, 'orderfield'=>$orderfield, 'direction'=>$direction, 'search'=>$search));
	}
