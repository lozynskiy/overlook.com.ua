		...
		if ($order_info) {
			...
			foreach ($products as $product) {
				...
				$data['products'][] = array(
					...
					'href'     		   => $this->url->linkToCatalog('product/product', '&product_id=' . $product['product_id'] . $url)
                        #$this->url->link('catalog/product/edit', 'token=' . $this->session->data['token'] . '&product_id=' . $product['product_id'], true)
				);
			}
