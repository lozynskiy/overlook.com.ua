...
class ControllerExtensionShippingXshippingpro extends Controller {
	...
	public function index() {   
		...
		$form_data = $this->getFormData($data);

		$data['form_data']=$form_data['method_html'];
		$data['methods']=$form_data['methods'];
	...
	public function quick_save() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$save=array();
			if(isset($this->request->post['xshippingpro']) && $this->request->get['tab_id']) {
	...
	private function getFormData($data)
	{
		...
		foreach($data['method_data'] as $single_method) {
			...
			.'</div>'
			.'</div>'
			.'<div class="form-group coupon-option" '.(($method_data['coupon_all']!='1')?'style="display:blocked"':'').'>'
			...
			for($i = 0; $i <= 23; $i++) { 
				$return.='<option '.(($method_data['time_start']==$i)?'selected':'').' value="'.$i.'">'.date("h:i A", strtotime("$i:00")).'</option>';
			} 
			$return.='</select>'
			.'</div>'
			.'<div class="col-sm-4">'
			...
			.'<option value="">'.$data['text_any'].'</option>';
			for($i = 0; $i <= 23; $i++) { 
				$return.='<option '.(($method_data['time_end']==$i)?'selected':'').' value="'.$i.'">'.date("h:i A", strtotime("$i:00")).'</option>';
			}
			$return.='</select>'
			.'</div>'
			.'</div>'
			...
			.'<div class="col-sm-10"><input type="text" name="xshippingpro[display]" value="'.$method_data['display'].'" class="form-control" id="input-display'.$no_of_tab.'" /></div>'
			.'</div>'
			.'</div>'
			.'</div>' 
			.'</div>';
		...
		return array('method_html'=>$return, 'methods' => $methods);		
	}
