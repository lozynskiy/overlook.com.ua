...
class ControllerCatalogSizechart extends Controller {
    ...
    public function add() {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
            ...
            $this->response->redirect($this->url->link('catalog/sizechart', 'token=' . $this->session->data['token'] . $url, true));
        }
    ...
    protected function getList() {
        ...
        $data['breadcrumbs'][] = array(
            ...
            'href' => $this->url->link('catalog/sizechart', 'token=' . $this->session->data['token'] . $url, true)
        );

        $data['add'] = $this->url->link('catalog/sizechart/add', 'token=' . $this->session->data['token'] . $url, true);
        $data['delete'] = $this->url->link('catalog/sizechart/delete', 'token=' . $this->session->data['token'] . $url, true);
        ...
        foreach ($results as $result) {
            $data['sizecharts'][] = array(
                ...
                'edit'            => $this->url->link('catalog/sizechart/edit', 'token=' . $this->session->data['token'] . '&sizechart_id=' . $result['sizechart_id'] . $url, true)
            );
        }
        ...
        $data['sort_name'] = $this->url->link('catalog/sizechart', 'token=' . $this->session->data['token'] . '&sort=name' . $url, true);
        $data['sort_sort_order'] = $this->url->link('catalog/sizechart', 'token=' . $this->session->data['token'] . '&sort=sort_order' . $url, true);
        ...
        $pagination->url = $this->url->link('catalog/sizechart', 'token=' . $this->session->data['token'] . $url . '&page={page}', true);
    ...
    protected function getForm() {
        ...
        if (!isset($this->request->get['sizechart_id'])) {
            $data['action'] = $this->url->link('catalog/sizechart/add', 'token=' . $this->session->data['token'] . $url, true);
        } else {
            $data['action'] = $this->url->link('catalog/sizechart/edit', 'token=' . $this->session->data['token'] . '&sizechart_id=' . $this->request->get['sizechart_id'] . $url, true);
        }

        $data['cancel'] = $this->url->link('catalog/sizechart', 'token=' . $this->session->data['token'] . $url, true);
        ...
        if (isset($this->request->post['sizechart_store'])) {
            $data['sizechart_store'] = $this->request->post['sizechart_store'];
        } elseif (isset($this->request->get['sizechart_id'])) {
            $data['sizechart_store'] = $this->model_catalog_sizechart->getSizechartStores($this->request->get['sizechart_id']);
        } else {
            $data['sizechart_store'] = array(0);
        }

        if (isset($this->request->post['image'])) {
            $data['image'] = $this->request->post['image'];
        } elseif (!empty($sizechart_info)) {
            $data['image'] = $sizechart_info['image'];
        } else {
            $data['image'] = '';
        }
        ...
        } elseif (!empty($sizechart_info) && is_file(DIR_IMAGE . $sizechart_info['image'])) {
            $data['thumb'] = $this->model_tool_image->resize($sizechart_info['image'], 100, 100);
        } else {
    ...
    protected function validateDelete() {
        ...
        foreach ($this->request->post['selected'] as $sizechart_id) {
            $product_total = $this->model_catalog_sizechart->getTotalProductsBySizechartId($sizechart_id);
